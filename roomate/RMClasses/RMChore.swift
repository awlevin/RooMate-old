...
public struct RMChore {
    ...
    static func updateChore(choreid: Int, beforePhoto: String, afterPhoto: String, completionHandler: (completed: Bool)->()) {
        let apiCallString = "https://damp-plateau-63440.herokuapp.com/updateRMChore"
        let httpURL = NSURL(string: apiCallString)
        let request = NSMutableURLRequest(URL: httpURL!)
